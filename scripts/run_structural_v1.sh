...
WORKROOT="$(dirname "$SOURCE_ROOT")/urban-city-generation-structural-v1"
CONFIG="$SOURCE_ROOT/configs/structural-v1.yaml"
PROGRAMS="$SOURCE_ROOT/data/programs/structural-v1"
RUN="$SOURCE_ROOT/runs/structural-v1"
SAMPLES="$SOURCE_ROOT/runs/structural-v1-samples"
AUDIT="$SOURCE_ROOT/runs/structural-v1-audit"
PACKAGE="$SOURCE_ROOT/runs/structural-v1-package"
ZIP="$SOURCE_ROOT/singapore-structural-v1.zip"
...
echo
echo "=== TRAIN STRUCTURAL GRAPH MODEL ==="
cd "$WORKROOT"
...
for spec in low:0.40 mid:0.70 high:0.95; do
    label="${spec%%:*}"
    temperature="${spec##*:}"
